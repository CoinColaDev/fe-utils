"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.default = void 0;

var _libRedirect;

function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

var _default = {
  libDownload: {
    backgroundColor: 'rgb(255, 255, 255)',
    borderTop: 'none',
    height: '70px',
    display: 'flex'
  },
  libClose: {
    width: '35px',
    height: '70px',
    lineHeight: '76px',
    fontWeight: 'lighter'
  },
  libInnerLogo: {
    margin: '10px 8px 0 0',
    width: '50px',
    height: '50px',
    backgroundImage: 'url(https://files-1251297012.cos.ap-hongkong.myqcloud.com/images/logo.png)'
  },
  libInnerContent: {
    flex: 1
  },
  libInnerTitle: {
    fontSize: '1rem',
    height: '1.5em',
    lineHeight: '1.5em',
    margin: 0,
    overflow: 'hidden',
    marginTop: '6px',
    marginBottom: '3px',
    color: 'rgb(51, 51, 51)'
  },
  libInnerDesc: {
    fontSize: ' 0.7rem',
    height: ' 1.5em',
    lineHeight: '1.5em',
    margin: 0,
    overflow: 'hidden',
    color: ' rgb(102, 102, 102)'
  },
  libRedirect: (_libRedirect = {
    fontSize: '0.8rem',
    width: '90px',
    backgroundImage: '-webkit-gradient(linear,left top,right top,from(#0080fe),to(#45beff))'
  }, _defineProperty(_libRedirect, "backgroundImage", 'linear-gradient(90deg,#0080fe,#45beff)'), _defineProperty(_libRedirect, "color", 'rgb(255, 255, 255)'), _defineProperty(_libRedirect, "display", '-webkit-box'), _defineProperty(_libRedirect, "WebkitBoxAlign", 'center'), _defineProperty(_libRedirect, "WebkitBoxPack", 'center'), _defineProperty(_libRedirect, "fontWeight", 'normal'), _defineProperty(_libRedirect, "textAlign", 'center'), _defineProperty(_libRedirect, "alignItems", 'center'), _defineProperty(_libRedirect, "justifyContent", 'center'), _defineProperty(_libRedirect, "height", '100%'), _libRedirect),
  libCloseBtn: {
    fontSize: '28px',
    width: '14px',
    height: '14px',
    lineHeight: '14px',
    marginLeft: '10.5px',
    content: " ",
    display: ' inline-block',
    boxSizing: 'content-box',
    backgroundImage: 'url("data:image/png;base64,")',
    backgroundRepeat: 'no-repeat',
    backgroundPosition: 'center',
    backgroundSize: 'cover'
  }
};
exports.default = _default;