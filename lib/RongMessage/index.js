...
var utils = {
  ...
  isTextMessage: function isTextMessage(msg) {
    return msg.messageType === 'TextMessage';
  },
  // 图片消息
  isImageMessage: function isImageMessage(msg) {
    return msg.messageType === 'ImageMessage';
  },
  // 文件消息
  isFileMessage: function isFileMessage(msg) {
    return msg.messageType === 'FileMessage';
  },
  isVideoMessage: function isVideoMessage(msg) {
    return msg.content.type === 'video';
  },
  isCommandMessage: function isCommandMessage(msg) {
    return msg.messageType === 'CommandMessage';
  },
  // 是否离线消息，表示在其它终端接收过
...
function connect(rongId, token, clearCache) {
  // 清除融云缓存信息，避免始终连不上
  if (clearCache) {
